...
@Component("reNonMonthResultStockInfoReader")
public class ReNonMonthResultStockInfoReader implements ItemReader<Set<StockInfo>> {

  protected static Logger logger = Logger.getLogger(ReNonMonthResultStockInfoReader.class);
  ...
  public Set<StockInfo> read() throws Exception {
    ...
    try {
      // rsltMonthChkDateがSG日より前の情報を1件取得
      // stockInfo = stockInfoRepos.findOneByRsltMonthChkDateBefore(chkDateBefore);
      // rsltMonthがNULLの情報を数件取得
      final Page<StockInfo> stockInfoPage =
          stockInfoRepos.findByRsltMonthChkDateBefore(new PageRequest(0, limit), chkDateBefore);
